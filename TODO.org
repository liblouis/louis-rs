...
- [X] Normalize input text to NFC Unicode form
  - Input is normalized to NFC at the start of the translation
    pipeline so users can supply NFD or NFC text and table authors
    need not add ~correct~ rules for combining characters.

...
- Evaluate NFD as a drop-in replacement for NFC in both ~pipeline.rs~ and
  ~trie.rs~. The key questions are: does word-boundary detection work
  correctly when a precomposed letter is split into base + combining mark?
  Does case-insensitive trie matching handle decomposed uppercase correctly?
